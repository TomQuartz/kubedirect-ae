...
function run_cmd {
    ...
    shift 2
    for baseline in $@; do
        for n_func in ${N_FUNCS[@]}; do
            eval "$cmd"
            cp ./result.log $RESULTS/$name.$baseline.$n_func.log
            cp ./stderr.log $RESULTS/stderr/$name.$baseline.$n_func.log
            sleep 30
        done
    done
}
...
###################### e2e ######################
cd $BASE_DIR/e2e
cmd="./run.sh \$baseline \$n_funcs \$n_pods"
